...
async function fetchListSongs() {
    ...
    try {
        const range = `${sheetName}!A2:D`; // Загружаем столбцы Name, Sheet, Index, Key
        const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${range}?key=${API_KEY}`;
        const response = await fetch(url);
        const data = await response.json();
...
function displayListSongs() {
    ...
    listSongs.forEach((song, index) => {
        ...
        songItem.textContent = `${song[0]} (${song[3]})`; // Название песни и сохранённая тональность
...
async function addSongToList() {
    ...
    if (!listSongs.some(s => s[0] === song.name && s[1] === song.sheet)) {
        listSongs.push([song.name, song.sheet, song.index, song.key]);

        // Обновляем данные в Google Sheets
        const range = 'listsongs!A2:D';
        const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${range}:append?valueInputOption=RAW&key=${API_KEY}`;
        const body = {
            values: [[song.name, song.sheet, song.index, song.key]]
        };
...
addToListButton.addEventListener('click', addSongToList);

document.addEventListener('DOMContentLoaded', async () => {
    await fetchListSongs(); // Загружаем данные из листа listsongs при старте
});
