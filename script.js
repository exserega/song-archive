...
async function addSongToList() {
  const sheetName = SHEETS[sheetSelect.value]; // Текущий лист
  const songIndex = songSelect.value; // Индекс выбранной песни
  ...
  const song = {
    name: songData[0], // Название песни
    sheet: sheetName, // Лист
    index: songIndex, // Индекс строки
    key: keySelect.value, // Тональность
    timestamp: new Date() // Дата добавления
  };

  try {
    await addDoc(collection(db, "songs"), song); // Добавляем песню в Firestore
...
async function loadListSongs() {
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      songItem.textContent = `${song.name} (${song.key})`; // Название песни и тональность

      // Кнопка удаления
      const removeBtn = document.createElement('span');
      removeBtn.textContent = '×';
      removeBtn.className = 'remove-btn';
      removeBtn.addEventListener('click', () => removeFromList(doc.id));

      songItem.appendChild(removeBtn);

      // Обработчик клика по песне
      songItem.addEventListener('click', () => {
        sheetSelect.value = song.sheet; // Выбираем лист
        songSelect.value = song.index; // Выбираем песню
    ...
    listContainer.innerHTML = '<div>Ошибка загрузки</div>';
  }
}

async function removeFromList(songId) {
  try {
    await deleteDoc(doc(db, "songs", songId)); // Удаляем песню из Firestore
    loadListSongs(); // Обновляем список
  } catch (error) {
...
document.getElementById('list-button').addEventListener('click', () => {
  ...
  loadListSongs(); // Загружаем список песен при открытии панели
});

document.addEventListener('DOMContentLoaded', async () => {
  await loadListSongs(); // Загружаем список песен при старте
});
